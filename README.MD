# ☃️ OLAV - Enterprise Network Operations AI Assistant

```
 ▄████▄   ██        ▄████▄    ██    ██
██▀  ▀██  ██       ██▀  ▀██   ██    ██
██    ██  ██       ████████   ██    ██
██▄  ▄██  ██       ██    ██    ██  ██
 ▀████▀   ████████ ██    ██    ████
```

**Version**: v0.8.0
**Status**: ✅ Production Ready
**Framework**: DeepAgents Native

OLAV is an intelligent, **agentic** network assistant designed for production environments. It bridges the gap between **Generative AI** and **Deterministic Network Automation** (Nornir/Netmiko).

**[中文指南](README_ZH.md)** | **English Guide**

---

## 🌟 Why OLAV?

### 🧠 Agentic & Autonomous
Built on the **DeepAgents** framework, OLAV isn't just a chatbot. It is a **goal-oriented autonomous agent** that can:
- **Plan**: Break down complex requests ("Audit all BGP neighbors") into sub-tasks with reasoning chains.
- **Act**: Execute multi-step workflows autonomously, using tools independently and validating results before proceeding.
- **Reflect**: Verify outputs against expected schemas, retry on failures, and adapt strategies based on feedback.
- **Memory**: Retain context across troubleshooting sessions, learning from past solutions and building institutional knowledge.
- **Self-Correcting**: Detect execution errors, analyze root causes, and autonomously adjust tactics—all without human intervention.

### 🔄 Claude Code Compatible
OLAV v0.8 adopts a 100% compatible file structure with **Claude Code** (`.claude/` ↔ `.olav/`).
- **Seamless Migration**: To use with Claude Code, simply copy `.olav/` to `.claude/` and rename `OLAV.md` to `CLAUDE.md`.
- **Standardized SOPs**: Use standard Markdown to define robust operational procedures that the Agent follows.

### 📊 Structured Output Parsing
Stop feeding raw text to LLMs.
- **TextFSM Templates**: OLAV automatically parses network CLI outputs through TextFSM templates, converting unstructured text streams into **Structured JSON schemas**.
- **Token Efficient**: This structured parsing approach enables the AI to reason about schema fields (`interface[0].status`, `bgp[0].state`) instead of parsing raw text, improving accuracy and reducing token usage significantly.

### 🔐 Schema-Aware Commands & API
Enforce type safety and parameter validation at execution boundaries.
- **Command Parameters**: All network commands are validated against strict schemas before execution—platform-specific (Cisco, Juniper, etc.) with typed parameters and validation rules.
- **API Validation**: RESTful APIs and tool calls enforce request/response schemas, preventing malformed commands and ensuring consistent output structure for downstream processing.
- **Type Safety**: Strong typing catches intent mismatches early, reducing runtime errors and improving agent reliability in autonomous execution.

### 🛡️ Enterprise Grade
- **3-Layer Knowledge Brain**: Decouples **Skills** (How), **Knowledge** (What), and **Capabilities** (Tools).
- **Human-in-the-Loop (HITL)**: Sensitive commands (`reload`, `config`) trigger mandatory approval gates—critical decisions require human validation.
- **Prompt Guard**: Automatic intent filtering blocks non-network queries, preventing prompt injection and off-topic requests.
- **Command Blacklisting**: Explicit blacklist of dangerous operations (reload, reboot, erase, etc.) enforced at the execution layer, preventing unintended operations.
- **Multi-Vendor**: Native support for Cisco, Huawei, Juniper, Arista, and any SSH-compatible device.

---

## 🚀 Quick Start

### 1. Installation
Prerequisites: Python 3.11+ (Required)

**Option A: Using uv (Recommended - Faster)**
```bash
# Install uv
pip install uv

# Install dependencies
uv sync

# Activate virtual environment (optional, uv run works without it)
source .venv/bin/activate  # On Linux/Mac
# or
.venv\Scripts\activate  # On Windows
```

**Option B: Using pip/virtualenv (Standard)**
```bash
# Create virtual environment
python3 -m venv .venv

# Activate virtual environment
source .venv/bin/activate  # On Linux/Mac
# or
.venv\Scripts\activate  # On Windows

# Install dependencies
pip install -e ".[dev]"
```

### 2. Configuration (Environment)
Create your configuration file from the template:

```bash
cp .env.example .env
```

Edit `.env` with your settings:
```bash
# Required: LLM Configuration
LLM_PROVIDER=openai  # or 'ollama' for local models
LLM_API_KEY=your-api-key-here  # Get from https://platform.openai.com or https://openrouter.ai

# Optional: For local Ollama
OLLAMA_BASE_URL=http://localhost:11434

# Optional: Embedding provider for RAG
EMBEDDING_PROVIDER=ollama
EMBEDDING_MODEL=nomic-embed-text:latest
```

### 3. Network Inventory Setup (Crucial)
OLAV uses Nornir to talk to your network. You must define your devices in `.olav/config/nornir/hosts.yaml`.
*Note: This is the only way OLAV knows which devices exist.*

1. Copy the example file:
   ```bash
   cp .olav/config/nornir/hosts.yaml.example .olav/config/nornir/hosts.yaml
   ```

2. Edit `hosts.yaml` with your actual device details. **This file is git-ignored** to protect credentials.

```yaml
# Example .olav/config/nornir/hosts.yaml
R1:  # <-- Alias used in Chat (e.g. "Check R1")
  hostname: 192.168.10.1
  platform: cisco_ios
  username: admin
  password: password123
  groups: ["core", "routers"]
```

### 4. Initialization (One-time)
Run the initialization script to set up all components automatically:

**If using uv:**
```bash
uv run python scripts/init.py
```

**If using pip/venv:**
```bash
python scripts/init.py
```

**What it does:**
- ✅ Generates `.olav/settings.json` from your `.env` values (model, temperature, etc.)
- ✅ Generates `.olav/knowledge/aliases.md` from your `hosts.yaml` device names
- ✅ Creates database schema for network data storage
- ✅ Creates `knowledge.db` schema for RAG search

**Additional options:**
```bash
# With uv
uv run python scripts/init.py --check           # Check initialization status
uv run python scripts/init.py --force           # Regenerate settings.json and aliases.md

# With pip/venv
python scripts/init.py --check
python scripts/init.py --force
python scripts/init.py --reload-commands
```

### 5. Launch

**Option A: Using uv**
```bash
uv run olav
```

**Option B: Using pip/venv (after activation)**
```bash
olav
# or
python -m olav
```

**Option C: Docker (For isolation or deployment)**
```bash
# Using OpenAI/Azure (recommended)
docker-compose up olav

# Using local Ollama LLM
docker-compose up

# Run interactively
docker-compose run --rm olav

# Pull latest Ollama model
docker-compose exec ollama ollama pull llama3
```

---

## 🔒 Security Features

OLAV implements a **defense-in-depth** security model to ensure safe autonomous operations in production networks.

### 1. **Prompt Guard** - Intent Filtering
Guards against prompt injection attacks and off-topic requests by validating that user queries are network-operations related.

- **Strict Mode**: Only explicit network operations are allowed (e.g., "Check BGP status")
- **Fuzzy Matching**: Intelligent classification to catch rephrasings of malicious prompts
- **Configurable**: Enable/disable via `guard.enabled` in `.olav/settings.json`

```json
{
  "guard": {
    "enabled": true,
    "strict_mode": false
  }
}
```

### 2. **Command Blacklisting** - Security Control

OLAV v0.9+ uses a **blacklist-only** approach for security, enforcing restrictions at the Nornir execution layer.

**Blacklist Location**: `.olav/config/guard_rules.yaml` (dangerous_commands pattern)

Example forbidden commands:
```yaml
dangerous_commands:
  - "^reload"
  - "^reboot"
  - "^erase"
  - "^format"
  - "^delete\\s+flash:"
  - "^write\\s+erase"
```

**How it works**:
1. User/Agent issues command
2. Guard checks against dangerous patterns (BEFORE LLM)
3. If matched → Block immediately with error message
4. If safe → Execute via Nornir

**Benefits**:
- ✅ Simplified: No need to maintain per-platform whitelists
- ✅ Flexible: Any command allowed except explicitly blocked
- ✅ Fail-safe: Enforced at lowest execution layer (can't bypass)
- ✅ Bilingual: Supports Chinese/English error messages

**Previous Approach (v0.8)**:
Used whitelist files (`.olav/imports/commands/{platform}.txt`) - removed in v0.9 for flexibility.

### 2b. **Guard Configuration** - Multi-Layer Security
### 2b. **Guard Configuration** - Multi-Layer Security

The Guard system provides multiple security checks:

**1. Pattern Matching** (Fast, Deterministic):
- Dangerous commands (reload, erase)
- Config changes (configure terminal, write memory)
- Sensitive data access (show run | include password)

**2. LLM Intent Classification** (Optional, Complex Scenarios):
- Analyzes natural language queries
- Detects suspicious intent patterns
- Requires approval for ambiguous requests

**Configuration**: `.olav/config/guard_rules.yaml`

See [Guard Rules Documentation](docs/guard_rules.md) for detailed configuration.

### 3. **Human-in-the-Loop (HITL)** - Approval Gates
Critical operations automatically pause for human approval before execution.

**Approval Required For**:
- **Write Operations**: Any command that modifies device state (config, reload)
- **Skill Updates**: When agent learns new troubleshooting solutions or device aliases
- **Risky Diagnosis**: High-impact diagnosis operations requiring manual sign-off

**Configuration** (`.olav/settings.json`):
```json
{
  "hitl": {
    "require_approval_for_write": true,
    "require_approval_for_skill_update": true,
    "approval_timeout_seconds": 300
  }
}
```

**Example Flow**:
```
User: "Update the VLAN config on SW1"
Agent: 🔐 HOLD - This is a write operation
       Please confirm: set vlan 100 name prod-vlan [Y/n]
User: Y
Agent: ✅ Proceeding with update...
```

---

## 💻 Workflows & CLI

OLAV operates in two modes: **Natural Language** (Agentic) and **Workflows** (Deterministic).

### Agentic Chat
Describe what you want, and the Agent plans the execution using its Skills.
> "Check why OSPF is flapping on the Core Router"
> "Audit VLAN consistency between Access-01 and Dist-01"

### Workflow Commands (`/`)
For frequent tasks, use Slash Commands to trigger **Schema-Aware Workflows**. These are faster, cheaper (fewer tokens), and more reliable than free-form chat.

**Core Commands:**

*   **`/snapshot`**: Captures network state snapshot with L1-L4 health analysis.
    *   *Example*: `/snapshot` (all devices) or `/snapshot group:core`
*   **`/analyze [query]`**: Starts the MicroAnalyzer sub-agent for deep diagnosis.
    *   *Example*: `/analyze "Why is BGP peering down on Core-01?"`
*   **`/search [query]`**: Hybrid search across knowledge base and web.
    *   *Example*: `/search "OSPF adjacency troubleshooting"`

| Command | Description | Agentic Level |
|---------|-------------|---------------|
| `/snapshot` | **Network Snapshot**: Full L1-L4 health check with topology visualization. | ⭐⭐ |
| `/analyze` | **Deep Diagnosis**: MicroAnalyzer sub-agent for root cause analysis. | ⭐⭐⭐ |
| `/search` | **RAG Search**: Hybrid search across Local Knowledge and Web. | ⭐ |
| `/devices` | **Inventory**: List connected devices and their connection status. | - |
| `/skills` | **Skill Inventory**: List loaded Markdown SOPs. | - |
| `/clear` | Clear screen. | - |
| `/quit` | Exit. | - |

---

## 🛠️ Advanced Features

### 1. Adding Skills (Claude Code Compatible)
Create a Markdown file in `.olav/skills/` to teach a new Standard Operating Procedure (SOP). OLAV natively reads **Claude Code** skill formats.

### 2. Configuring TextFSM Parsers
To teach OLAV how to parse a new network command:
1.  Drop the NTC-template file into `.olav/config/textfsm/`.
2.  Update the index file at `.olav/config/textfsm/index`.

OLAV will automatically use the template to convert command output to JSON, enabling the agent to reason about structured data instead of raw text.

### 3. Adding Workflows
To expose a Skill as a Slash Command (Workflow):
1. Create the skill markdown as above.
2. Register it in `.olav/config/commands.yaml` (or simply rely on the auto-loader for search).
*Note: High-priority workflows like `/analyze` are hardcoded for performance.*

### 4. Knowledge Base (RAG) & Standard Paths
OLAV follows a **"Documents as Truth"** philosophy.

**Default File Locations:**
| Function | I/O | Path | Format |
|---|---|---|---|
| **Knowledge Base** | Input | `.olav/knowledge/` | `.md`, `.txt`, `.pdf` |
| **Snapshot Reports** | Output | `exports/reports/snapshots/` | `.md` (Daily health reports) |
| **Topology** | Output | `exports/visualizations/` | `.html` (Interactive topology) |
| **Skills** | Input | `.olav/skills/` | `.md` (Claude Code format) |
| **Inventory** | Input | `.olav/config/nornir/hosts.yaml` | YAML |

**Internal Data (Advanced):**
| Function | Path | Description |
|---|---|---|
| **Sync Cache** | `data/sync/{date}/` | Raw/Parsed device output (intermediate data) |

**Usage:**
- Place local site docs into `knowledge/` and run `uv run python scripts/index_knowledge.py`.
- Find generated snapshot reports in `exports/reports/snapshots/YYYYMMDD.md`.
- View network topology in `exports/visualizations/topology.html`.

### 5. Scheduled Snapshots (Cron)

OLAV supports automated scheduled snapshots using your operating system's native task scheduler. This is more reliable than in-process schedulers and survives system restarts.

**Linux/macOS (crontab):**
```bash
# Edit crontab
crontab -e

# Add scheduled snapshot (daily at 6:00 AM)
0 6 * * * cd /path/to/olav && uv run olav query "/snapshot" >> .olav/logs/cron.log 2>&1

# Alternative: Weekly report (Monday at 9:00 AM)
0 9 * * 1 cd /path/to/olav && uv run olav query "/snapshot --report" >> .olav/logs/cron.log 2>&1
```

**Windows (Task Scheduler):**
1. Open **Task Scheduler** (taskschd.msc)
2. Click **Create Basic Task**
3. Set trigger: Daily at 6:00 AM
4. Set action: Start a program
   - Program: `cmd.exe`
   - Arguments: `/c cd /d C:\path\to\olav && uv run olav query "/snapshot"`
5. Enable "Run whether user is logged on or not"

**Output Location:**
```
exports/
├── reports/
│   └── snapshots/
│       └── 20260114.md      # Daily snapshot report
└── visualizations/
    └── topology.html        # Network topology
```

**Cron Schedule Syntax:**
```
┌───────────── minute (0-59)
│ ┌───────────── hour (0-23)
│ │ ┌───────────── day of month (1-31)
│ │ │ ┌───────────── month (1-12)
│ │ │ │ ┌───────────── day of week (0-6, Sunday=0)
│ │ │ │ │
* * * * * command
```

Common examples:
- `0 6 * * *` - Daily at 6:00 AM
- `0 */4 * * *` - Every 4 hours
- `0 9 * * 1` - Every Monday at 9:00 AM
- `0 0 1 * *` - First day of each month at midnight

### 6. Agentic Self-Learning
OLAV can learn from successful troubleshooting sessions and user interactions:

**Auto-Generated Content:**
| What | Path | Trigger |
|------|------|---------|
| **Solutions** | `.olav/knowledge/solutions/*.md` | After successful diagnosis, agent saves the case |
| **Device Aliases** | `.olav/knowledge/aliases.md` | When user clarifies device names (e.g., "Core router is R1") |

**Example - Learning Aliases:**
```
User: "Core switches are SW1 and SW2"
Agent: ✓ Learned alias "Core switches" → "SW1, SW2"
```
The agent appends to `aliases.md` and will use this knowledge in future queries.

**Example - Saving Solutions:**
After resolving a BGP flapping issue, the agent auto-saves:
```markdown
# Case: bgp-flapping-r1
> Created: 2026-01-12 (Auto-saved by OLAV)

## Problem
BGP neighbor flapping on R1...

## Root Cause
MTU mismatch on transit link...

## Solution
Adjusted MTU to 1500...
```

*Note: Solution auto-save requires `learning.autoSaveSolutions: true` in settings.json*

---

## 🏗️ Architecture

- **Runtime**: Python 3.12 + `uv`
- **Agent Framework**: DeepAgents
- **Database**: DuckDB (Vector + Relational)
- **Execution**: Nornir (Concurrency) + Netmiko (SSH)
- **Protocols**: SSH, Telnet, NETCONF

## 📄 License & Support

**MIT License** - Open Source & Enterprise Ready.
