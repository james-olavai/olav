...
# =============================================================================
# TextFSM Custom Templates Support
# =============================================================================


def _setup_custom_textfsm_templates() -> None:
    """Setup custom TextFSM templates directory if it exists.

    Netmiko will search for templates in this order:
    1. NET_TEXTFSM environment variable directory (custom templates)
    2. ntc-templates package directory (built-in templates)

    This allows users to add custom templates or override built-in ones.
    """
    from config.logging import get_logger
    from config.settings import PROJECT_ROOT

    logger = get_logger(__name__)
    custom_templates = PROJECT_ROOT / "config" / "templates"

    if custom_templates.exists():
        os.environ["NET_TEXTFSM"] = str(custom_templates.absolute())
        logger.info(f"已加载自定义 TextFSM 模板目录: {custom_templates}")
    else:
        # 创建示例目录结构
        custom_templates.mkdir(parents=True, exist_ok=True)
        readme = custom_templates / "README.md"
...
""",
            ...
            logger.info(f"已创建自定义模板目录: {custom_templates}")
...
@tool
def sync_all(
    devices: str = "all", group: str = "test", categories: str | list[str] | None = None
) -> str:
    ...
    try:
        # Setup custom TextFSM templates (if exists)
        _setup_custom_textfsm_templates()
        ...
        # 注意: 使用精确命令名避免匹配到带通配符的旧命令 (如 show interface*)
        skill_command_intents = {
            ...
            "interfaces": ["show ip interface brief", "show interface status"],
        ...
        for device_name in device_names:
            ...
            device_commands = [
                ...
                or platform in cmd["platform"]
                or cmd["platform"] in platform
            ]
            ...
            for cmd_info in device_commands:
                ...
                try:
                    ...
                    # use_textfsm=True: Netmiko will auto-parse using ntc-templates
                    result = nr.filter(name=device_name).run(
                        task=netmiko_send_command,
                        command_string=command,
                        read_timeout=10,
                        on_failed=True,
                        use_textfsm=True,  # 启用 TextFSM 自动解析
                    )
                    ...
                    if device_name in result and not result[device_name].failed:
                        ...
                        # Get result - could be parsed (list/dict) or raw (str)
                        output_data = result[device_name].result

                        # Save based on result type
                        if isinstance(output_data, (list, dict)):
                            # TextFSM parsed successfully - save as JSON
                            import json

                            parsed_file = sync_dir / "parsed" / device_name / f"{cmd_safe}.json"
                            parsed_file.write_text(
                                json.dumps(output_data, indent=2), encoding="utf-8"
                            )

                            # Also save raw for reference (use scrapli_send_command separately if needed)
                            # For now, we only keep parsed data
                        else:
                            # TextFSM failed or no template - save as raw text
                            output_file = sync_dir / "raw" / device_name / f"{cmd_safe}.txt"
                            output_file.write_text(str(output_data), encoding="utf-8")
                        ...
                        _store_sync_output(
                            ...
                            output_size=(
                                parsed_file.stat().st_size
                                if isinstance(output_data, (list, dict))
                                else output_file.stat().st_size
                            ),
                        )
...
def _generate_inspection_analysis_report(sync_dir: Path, device_names: list[str]) -> None:
    ...
    try:
        ...
        if l1_score < 100:
            ...
            report_lines.append(
                "> If links remain stable for 24 hours, score will recover on next check."
            )
            report_lines.append("")
        ...
        if l1_score < 80:
            ...
            report_lines.append("")
            report_lines.append(
                "**Issue**: Detected frequent interface state changes (link flapping)"
            )
            report_lines.append("")
        ...
        if high_event_cats:
            ...
            report_lines.append("")
            report_lines.append(
                f"**Issue**: High frequency events in categories: {', '.join(high_event_cats)}"
            )
            report_lines.append("")
            report_lines.append("**Recommended Commands**:")
            report_lines.append("```bash")
...
def _parse_with_textfsm(command: str, output: str, platform: str) -> list[dict]:
    ...
    elif "show ip bgp summary" in command_normalized:
        return _parse_bgp_summary(output)
...
def _parse_bgp(output: str) -> list[dict]:
    """Parse BGP routing table (show ip bgp)."""
    results = []
    lines = output.strip().split("\n")
...
def _parse_bgp_summary(output: str) -> list[dict]:
    """Parse BGP summary (show ip bgp summary).

    Example output:
        Neighbor        V           AS MsgRcvd MsgSent   TblVer  InQ OutQ Up/Down  State/PfxRcd
        3.3.3.3         4        65000     654     658       10    0    0 09:50:31        1
        10.1.12.2       4        65001     658     650       10    0    0 09:50:34        1
    """
    results = []
    lines = output.strip().split("\n")
    ...
    for line in lines:
        ...
        # Skip empty lines and BGP router info
        if not line:
            continue

        # Find Neighbor header
        if line.startswith("Neighbor"):
            header_found = True
            continue

        # Parse data after header
        if header_found and not line.startswith(("BGP", "Status")):
            parts = line.split()
            if len(parts) >= 10:  # At least 10 fields
                neighbor_ip = parts[0]
                version = parts[1]
                remote_as = parts[2]
                msg_rcvd = parts[3]
                ...
                # State/PfxRcd: number (Established) or status text
                try:
