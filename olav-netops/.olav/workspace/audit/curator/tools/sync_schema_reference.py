#!/usr/bin/env python3
"""sync_schema_reference — Auto-sync SCHEMA_REFERENCE.md with live DuckDB schema.

Regenerates the "Core Table & View Schema" section of a Quick Agent's
SCHEMA_REFERENCE.md by querying ``information_schema.columns`` for each of
the declared core objects. All human-written sections (SQL examples, common
mistakes, fallback rule) are preserved unchanged.

Intended trigger points
-----------------------
1. After ``IngestManager.bulk_load()`` via its ``post_ingest_hooks`` list.
2. Manually via ``olav config`` agent ("sync schema reference").
3. CLI: ``python sync_schema_reference.py`` (reads optional JSON from stdin).

Returns
-------
``{"status": "updated" | "up_to_date" | "error", "message": "..."}``

``status="updated"`` — file was changed, diff included in ``message``.
``status="up_to_date"`` — file content already matched live DB; not written.
``status="error"`` — DB unreachable or section heading not found in file.
"""
...
def main(args: dict[str, Any]) -> dict[str, Any]:
    ...
    from olav.core.config import MAIN_DB_PATH  # noqa: PLC0415  (local import intentional)

    db_path: str = args.get("db_path") or str(MAIN_DB_PATH)
    schema_ref_path: str = args.get("schema_ref_path") or str(
        project_root
        / ".olav"
        / "workspace"
        / "quick"
    ...
    return _sync(db_path=db_path, schema_ref_path=schema_ref_path)
...
def sync_schema_reference(
    ...
    ``information_schema.columns`` for every core table and view. All
    human-written sections (SQL examples, common mistakes, fallback rule)
    are preserved unchanged.  Safe to run repeatedly — idempotent.

    Args:
        db_path: Path to main.duckdb. Defaults to ``PathsConfig().MAIN_DB_PATH``.
        schema_ref_path: Path to SCHEMA_REFERENCE.md to update. Defaults to
    ...
    return main({
        ...
        "schema_ref_path": schema_ref_path or None,
    })
...
if __name__ == "__main__":
    ...
    try:
        ...
        input_data: dict[str, Any] = json.loads(raw) if raw else {}
