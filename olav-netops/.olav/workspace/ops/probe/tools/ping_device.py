...
def ping_device(
    ...
    # Resolve hostname → management IP from devices table
    if not re.match(r'^\d+\.\d+\.\d+\.\d+$', target):
        try:
            ...
            _rconn = _ddb.connect(str(MAIN_DB_PATH), read_only=True)
            _row = _rconn.execute(
                "SELECT mgmt_ip FROM devices WHERE LOWER(name) = LOWER(?)",
                [target],
            ).fetchone()
            _rconn.close()
            if _row and _row[0]:
            ...
            pass  # fall through, let ping attempt the hostname as-is
