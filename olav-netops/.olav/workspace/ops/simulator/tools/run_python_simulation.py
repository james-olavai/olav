...
def run_python_simulation(
    ...
    # NOTE: link_status values are 'up' / 'down' (not 'active')
    links = sim.execute("SELECT source_device, destination_device FROM sim_topology_links WHERE link_status = 'up'").fetchall()
    G = nx.DiGraph()
