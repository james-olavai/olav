...
class SuzieQTool:
    ...
    def _read_parquet_table(self, table_dir: Path) -> Optional[pd.DataFrame]:
    ...
    def _filter_by_time_window(
        ...
        logger.info(
            f"Time window filter: {len(filtered)}/{len(df)} records "
            f"(last {max_age_hours}h)"
        )
        return filtered
    
    def _deduplicate_latest_state(
        self,
        df: pd.DataFrame,
        table: str
    ) -> pd.DataFrame:
...
class SuzieQSchemaSearchTool:
    ...
    async def execute(self, query: str) -> ToolOutput:
        ...
        for table, schema in SUZIEQ_SCHEMA.items():
            if any(
                kw in table.lower() or kw in schema["description"].lower()
                for kw in keywords
            ):
                matches.append({
                    "table": table,
