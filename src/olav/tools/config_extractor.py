...
class ConfigSectionExtractor:
    ...
    def extract_for_diagnosis(
        ...
        section_keywords: dict[SectionType, list[str]] = {
            "route-map": ["route-map", "route map", "routing policy", "policy"],
            "prefix-list": ["prefix-list", "prefix list"],
            "bgp": ["bgp", "as number", "autonomous system", "neighbor"],
            "bgp-neighbor": ["neighbor", "peer"],
        ...
        if any(kw in hypothesis_lower for kw in ["route", "routing", "policy", "block"]):
