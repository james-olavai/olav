...
class DiffEngine:
    ...
    async def _compare_interfaces(
        ...
        """Compare interface data between SuzieQ and NetBox."""
        ...
        for device in devices:
            try:
                ...
                if netbox_result.error:
                    logger.warning(f"NetBox interface query failed for {device}: {netbox_result.error}")
                    continue
                ...
                # Compare
                self._diff_interfaces(device, suzieq_interfaces, netbox_interfaces, report)
    ...
    def _parse_suzieq_addresses(
        ...
        """Parse SuzieQ address query result."""
        addresses = {}
        ...
        for row in data:
            if row.get("hostname") == device:
                ip = row.get("ipAddressList", [""])[0] if row.get("ipAddressList") else ""
                if ip:
                    addresses[ip] = {
