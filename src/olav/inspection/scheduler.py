...
class InspectionScheduler:
    ...
    async def _run_loop(self) -> None:
        ...
        if InspectionConfig.SCHEDULE_INTERVAL_MINUTES:
            ...
            logger.info(f"Running inspections every {InspectionConfig.SCHEDULE_INTERVAL_MINUTES} minutes")
    ...
    async def _run_daily_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            logger.info(f"Next inspection scheduled at {next_run.strftime('%Y-%m-%d %H:%M:%S')} ({wait_seconds/3600:.1f} hours)")
