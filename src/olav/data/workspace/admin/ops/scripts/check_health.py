...
def _check_workspace() -> dict:
    """Check olav.md registration and all agent/skill/tool validity."""
    results: dict = {"status": "ok", "checks": [], "agents": []}

    platform_md = _WORKSPACE / "olav.md"
    if not platform_md.exists():
        results["checks"].append({"name": "olav.md", "status": "error",
                                   "message": "Missing .olav/workspace/olav.md"})
    ...
    try:
        ...
        results["checks"].append({"name": "olav.md parse", "status": "error",
                                   "message": str(e)})
        results["status"] = "error"
        return results

    results["checks"].append({"name": "olav.md", "status": "ok",
                               "registered_agents": registered})

    # Detect workspace dirs not in olav.md
    workspace_dirs = {
            ...
            if active_ws:
                ...
                if active_ws in registered:
                    ...
                    results["checks"].append({"name": "Active workspace", "status": "warning",
                                               "message": f"'{active_ws}' not in olav.md agents"})
                    if results["status"] == "ok":
                        results["status"] = "warning"
...
def _generate_recommendations(health: dict) -> list[str]:
    ...
    for check in health.get("workspace", {}).get("checks", []):
        if check.get("status") == "warning" and "Unregistered" in check.get("name", ""):
            recs.append(f"Register agents in olav.md: {check.get('agents', [])}")
...
def check_health(
    ...
    Validates:
    - olav.md agent registry completeness
