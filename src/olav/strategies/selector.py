...
class StrategyDecision(BaseModel):
    ...
    reasoning: str
    fallback: Optional[Literal["fast_path", "deep_path", "batch_path"]] = None
...
class StrategySelector:
    ...
    async def select(self, user_query: str) -> StrategyDecision:
        ...
        if self.use_llm_fallback:
            ...
            llm_decision = await self._llm_based_selection(user_query, rule_decision)
            return llm_decision
        
        # No LLM available, return rule-based decision
        logger.warning(
            f"Low confidence ({rule_decision.confidence:.2f}) but no LLM fallback"
        )
        return rule_decision
    ...
    def _rule_based_selection(self, user_query: str) -> StrategyDecision:
        ...
        batch_keywords = [
            "batch", "批量", "all devices", "所有设备",
            "compliance", "合规", "audit", "审计",
            "health check", "健康检查", "inspect all", "检查所有",
            "validate all", "验证所有", "all routers", "所有路由器",
            "all switches", "所有交换机", "every device", "每个设备"
        ]
        ...
        diagnostic_keywords = [
            "why", "为什么", "diagnose", "诊断", "troubleshoot", "排查",
            "analyze", "分析", "investigate", "调查", "root cause", "根因",
            "explain", "解释", "debug", "调试", "trace", "追踪"
        ]
        ...
        fast_keywords = [
            "show", "显示", "list", "列出", "get", "获取",
            "查询", "query", "check", "检查", "status", "状态",
            "what is", "什么是", "how many", "多少"
        ]
    ...
    async def _llm_based_selection(
        self,
        user_query: str,
        rule_decision: StrategyDecision
    ) -> StrategyDecision:
        """
...
"""
        ...
        try:
            ...
            logger.info(
                f"LLM classification: {decision.strategy} "
    ...
    def get_strategy_class(
        strategy_name: Literal["fast_path", "deep_path", "batch_path"]
    ) -> Type:
        """
