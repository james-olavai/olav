...
from olav.schemas.inspection import InspectionConfig, CheckTask
from olav.validation.threshold import ThresholdValidator, DeviceValidationResult
...
class DeviceCheckResult(BaseModel):
    ...
    validation: Optional[DeviceValidationResult] = None
...
class BatchPathResult(BaseModel):
    ...
    violations: List[str] = Field(
        ...
        description="All violation messages"
    )
...
class BatchPathStrategy:
    ...
    async def execute(
        ...
        if config.parallel:
            results = await self._execute_parallel(
                devices, enabled_checks, config.max_workers
            )
        else:
    ...
    async def _resolve_devices(self, config: InspectionConfig) -> List[str]:
        ...
        if config.devices.explicit:
        ...
        elif config.devices.netbox_filter:
            ...
            devices = [device.get("name", "") for device in result.data if device.get("name")]
            return devices
        
        elif config.devices.regex:
            ...
            if suzieq_tool:
                result = await suzieq_tool.execute(table="device", method="unique", column="hostname")
    ...
    async def _execute_parallel(
        ...
        tasks = [
            execute_with_semaphore(device, check)
    ...
    def is_suitable(query: str) -> bool:
        ...
        batch_keywords = [
            ...
            "health check", "健康检查", "inspect", "检查",
            "validate", "验证", "all routers", "所有路由器",
            "all switches", "所有交换机"
        ]
