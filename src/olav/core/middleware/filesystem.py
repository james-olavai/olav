...
class FilesystemMiddleware:
    ...
    def _validate_path(
        ...
        try:
            ...
            raise ValueError(
                f"Path escapes workspace root: {normalized} not in {self.workspace_root}"
            ) from e
        ...
        if allowed_prefixes:
            ...
            if not any(str(relative).startswith(prefix) for prefix in allowed_prefixes):
                raise ValueError(
                    f"Path not in allowed prefixes: {relative} not in {allowed_prefixes}"
                )
    ...
    async def write_file(
        ...
        if not approved:
            ...
            raise ValueError(f"HITL approval denied for write: {path}")
    ...
    async def delete_file(
        ...
        if not approved:
            ...
            raise ValueError(f"HITL approval denied for delete: {path}")
