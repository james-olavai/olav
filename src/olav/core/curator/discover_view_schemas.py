...
def discover_view_schemas(
    ...
    with duckdb.connect(str(MAIN_DB_PATH)) as con:
        ...
        # RAW-05: union raw_output_store into the candidate set so commands
        # that only have raw captures (e.g. Junos outputs that failed TextFSM
        # parsing) still surface for view-recipe discovery. The parsed-data
        # sampling block below skips entries without a usable JSON payload.
        if force_refresh:
            ...
            new_commands = con.execute("""
                ...
                SELECT command FROM candidates c
                WHERE NOT EXISTS (
                    SELECT 1 FROM view_recipes r WHERE r.command = c.command
                )
                ORDER BY command
            """).fetchall()
