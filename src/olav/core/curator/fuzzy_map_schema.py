    ...
    try:
        with duckdb.connect(str(MAIN_DB_PATH)) as conn:
            ...
            sql = """
                ...
                JOIN devices d ON p.device_name = d.name
...
def fuzzy_map_schema(
    ...
    Uses an LLM to translate vendor-specific JSON keys to standard
    keys (e.g. Cisco IOS baseline), caches the mapping for reuse.
    """
    return main(
        {
            "vendor": vendor,
