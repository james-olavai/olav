...
class ThresholdAgent:
    ...
    async def _learn_threshold(
        self, device: str, metric_name: str, metric_cfg: dict[str, Any]
    ) -> dict[str, Any] | None:
        ...
        # Table and column names are from hardcoded mapping above (safe)
        sql = f"""
            SELECT {column}
            FROM main.{table}
            WHERE device = ?
        ...
        try:
            ...
            arr = np.array(data)
            learned: dict[str, str | float] = {"last_learned": "2026-01-29T11:00:00+11:00"}
