...
class DynamicIntentRouter:
    ...
    async def build_index(self) -> None:
        ...
        for metadata in workflows:
            if not metadata.examples:
                logger.warning(
                    f"Workflow '{metadata.name}' has no examples, "
                    f"using description for embedding"
                )
    ...
    async def semantic_prefilter(self, query: str) -> List[Tuple[str, float]]:
        ...
        if not self.example_vectors:
            raise RuntimeError(
                "Semantic index not built. Call build_index() first."
            )
...
"""
    ...
    async def route(
        ...
        if trigger_matches:
            logger.info(
                f"Trigger match found for query: {trigger_matches[0]} "
                f"(skipping semantic routing)"
            )
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            "top_k": self.top_k,
            "embedding_dimension": (
                len(next(iter(self.example_vectors.values())))
                if self.example_vectors else 0
            ),
        }
