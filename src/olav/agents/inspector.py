...
class ReducePhase:
    ...
    async def analyze_global_anomalies(self, anomalies: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
...
"""
        ...
        try:
            ...
            return {
                "root_cause": "LLM 分析失败。",
                "impact": "无法评估。",
                "recommendations": [{"priority": "warning", "action": "请检查 LLM 连接或手动分析异常数据。"}]
            }
    ...
    def _prepare_vars(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for device in all_devices:
            ...
            for a in device_anomalies:
                ...
                if "L1" in layer: l1 = icon
                elif "L2" in layer: l2 = icon
                elif "L3" in layer: l3 = icon
                else: l4 = icon
        ...
        return {
            ...
            "immediate_actions": [r["action"] for r in result["llm_analysis"].get("recommendations", []) if r.get("priority") == "critical"],
            "planned_actions": [r["action"] for r in result["llm_analysis"].get("recommendations", []) if r.get("priority") == "warning"],
            "optimization_suggestions": [r["action"] for r in result["llm_analysis"].get("recommendations", []) if r.get("priority") == "info"]
        }
...
class InspectionOrchestrator:
    ...
    async def run_inspection(self, test_mode: bool = False, device_filter: List[str] | None = None) -> str:
        ...
        # 6. Render Report
        report_skill_id = inspection_skill.frontmatter.get("output", {}).get("report_skill", "inspect-report")
    ...
    def _calculate_summary(self, total_devices: int, anomalies: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
        critical_count = sum(1 for d in anomalies.values() if any(a["severity"] == "critical" for a in d))
        warning_count = sum(1 for d in anomalies.values() if any(a["severity"] == "warning" for a in d) and not any(a["severity"] == "critical" for a in d))
    ...
    def _save_report(self, report: str):
        ...
        report_path = REPORTS_DIR / "inspection" / f"report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
