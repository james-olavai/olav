...
async def generate_node(state: CoderState) -> CoderState:
...
async def test_node(state: CoderState) -> CoderState:
...
async def analyze_node(state: CoderState) -> CoderState:
...
def create_coder_graph() -> StateGraph:
...
def create_coder_agent(model: str = "gpt-4o") -> Callable[[str, str, str], Awaitable[str]]:
