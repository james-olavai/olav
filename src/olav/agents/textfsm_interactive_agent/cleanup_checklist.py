"""
TextFSM Agent Migration & Cleanup Checklist

Validates completeness of migration from old LangGraph implementation to
new interactive DeepAgents implementation.
"""
...
class TextFSMCleanupChecklist:
    """
    Verifies migration completeness and validates local environment.
    
    Checks:
    1. New agent module is properly importable
    ...
    async def _check_old_agent_replacement(self) -> bool:
        """Check 3: Old agent replaced or marked for deprecation."""
        ...
        if not old_agent_path.exists():
            logger.info("  ✓ Old textfsm_agent.py not found (likely already removed)")
            self.checks_passed += 1
            return True
        
        # Check if old agent has deprecation notice
        try:
            with open(old_agent_path, "r") as f:
                content = f.read()
            
            if "deprecated" in content.lower() or "textfsm_interactive_agent" in content:
                logger.info("  ✓ Old agent marked as deprecated/replaced")
                self.checks_passed += 1
                return True
            else:
                self.warnings.append(
                    "Old textfsm_agent.py exists but not marked as deprecated. "
                    "Consider adding deprecation notice."
                )
                logger.warning(
                    "  ⚠ Old agent exists but not marked as deprecated"
                )
                return True  # Not a blocker
        
        except Exception as e:
            logger.error(f"  ✗ Error checking old agent: {e}")
            self.checks_failed += 1
            return False
    ...
    async def _check_backward_compatibility(self) -> bool:
        ...
        # Check that old SKILL.md still works
        skill_path = (
            self.workspace_root / ".olav" / "skills" / "textfsm-generator" / "SKILL.md"
        )
        
        if skill_path.exists():
            logger.info(f"  ✓ Found existing SKILL.md at {skill_path}")
            logger.info("  ✓ Old skill configuration preserved")
        else:
            logger.warning("  ⚠ No existing SKILL.md found (first-time setup)")
