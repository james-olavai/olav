...
Everything else (scripts, dev_docs, sub-repos, exports, databases) stays on disk for development but is not in the release.

`tests/**` is **tracked in git** (policy flipped 2026-06-13) but excluded
from the release wheel by the packaging rule (`packages = ["src/olav"]`).
It was gitignored until then, which left 57% of test files (198/349)
invisible to CI and code review — release hygiene is the wheel's job,
not gitignore's. When adding a test file, plain `git add` works; no
`-f` needed anymore.
