...
# ─────────────────────────────────────────────────────────
# C-L2-21  知识库索引与语义搜索
# Claim: Knowledge Base 支持文档索引和语义搜索
# Doc:   docs/guides/knowledge-base.md
# ─────────────────────────────────────────────────────────
class TestKnowledgeBaseClaim:
    """
    Claim C-L2-21: KnowledgeBase 可索引文档并支持语义搜索。
    Verified: 2026-04-03 | Doc: docs/guides/knowledge-base.md
    """

    def test_kb_index_returns_success(self):
        ...
        result = kb.index_document(P("test_ddd.md"), "BGP is an exterior gateway protocol (EGP).")
    ...
    def test_kb_search_returns_results(self):
        ...
        store = LanceDBStore()
        kb = KnowledgeBase(store)
        kb.index_document(P("test_ddd_ospf.md"), "OSPF is a link-state routing protocol.")
        results = kb.search("OSPF routing protocol")
        assert len(results) > 0

    def test_kb_search_result_has_text(self):
        from olav.core.memory import LanceDBStore
        from olav.core.knowledge import KnowledgeBase
        ...
        if results:
            assert "text" in results[0] or "chunk" in results[0] or isinstance(results[0], dict)
