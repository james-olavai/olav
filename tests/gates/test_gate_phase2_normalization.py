"""Phase 2 Gate: Data Normalization (OpenConfig).

Gate tests verify static post-conditions against a running DB state — they are
NOT end-to-end tests that exercise the normalization pipeline. Run them after
a real normalization cycle to confirm the pipeline produced correct output.
...
Run with:
    uv run pytest tests/gates/test_gate_phase2_normalization.py -v
