...
def test_run_single_query_audit_chain(audit_db, monkeypatch):
    ...
    # NOTE: pre-v0.20 the test also patched
    # ``deepagents_cli.execution.execute_task``; that submodule was
    # removed in deepagents-cli 0.0.41, and ``run_single_query`` now
    # talks to the graph directly via ``astream_events`` — the patch is
    # no longer needed, so it was dropped.
    with patch("olav.cli.main.create_olav_agent_with_backend") as mock_create:
        mock_agent = MagicMock()
        # run_single_query calls `_graph.astream_events(...)`; give the
        # mock graph an async-iterable stub so the call returns cleanly
        ...
        mock_agent.astream_events = _empty_events
        mock_agent.graph.astream_events = _empty_events
