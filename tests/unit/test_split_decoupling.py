...
class TestCore2EnterpriseDepsOptional:
    ...
    def test_tink_in_enterprise_optional(self):
        """tink must appear under the enterprise optional extra."""
        src = self._pyproject_src()
        lines = src.splitlines()
        in_enterprise = False
        for line in lines:
            stripped = line.strip()
            if "enterprise = [" in stripped or "enterprise=[" in stripped:
                in_enterprise = True
            if in_enterprise and "tink" in stripped:
                return  # Found — pass
            if in_enterprise and stripped == "]":
                in_enterprise = False
        pytest.fail("tink not found in enterprise optional-dependencies section")

    def test_pyjwt_in_enterprise_optional(self):
        """PyJWT must appear under the enterprise optional extra."""
        src = self._pyproject_src()
        lines = src.splitlines()
        in_enterprise = False
        for line in lines:
            ...
            if "enterprise = [" in stripped or "enterprise=[" in stripped:
                in_enterprise = True
            if in_enterprise and "PyJWT" in stripped:
                return  # Found — pass
            if in_enterprise and stripped == "]":
                in_enterprise = False
        pytest.fail("PyJWT not found in enterprise optional-dependencies section")


# ===========================================================================
# NETOPS-1: Workspace asset manifest must exist and document ownership
# ===========================================================================


class TestNetops1WorkspaceManifest:
    """NETOPS_WORKSPACE_MANIFEST.md must exist and document asset ownership."""

    def test_manifest_file_exists(self):
        """dev_docs/NETOPS_WORKSPACE_MANIFEST.md must exist."""
        assert _NETOPS_MANIFEST.exists(), (
            "dev_docs/NETOPS_WORKSPACE_MANIFEST.md not found — NETOPS-1 not complete"
        )

    def test_manifest_mentions_ops_workspace(self):
        """Manifest must document .olav/workspace/ops ownership."""
        src = _NETOPS_MANIFEST.read_text(encoding="utf-8")
        assert ".olav/workspace/ops" in src, (
            "NETOPS_WORKSPACE_MANIFEST.md must document .olav/workspace/ops asset"
        )

    def test_manifest_mentions_config_sync(self):
        """Manifest must document .olav/workspace/config/sync ownership."""
        src = _NETOPS_MANIFEST.read_text(encoding="utf-8")
        assert "config/sync" in src, "NETOPS_WORKSPACE_MANIFEST.md must document config/sync asset"

    def test_manifest_mentions_config_learner(self):
        """Manifest must document .olav/workspace/config/learner ownership."""
        src = _NETOPS_MANIFEST.read_text(encoding="utf-8")
        assert "config/learner" in src, (
            "NETOPS_WORKSPACE_MANIFEST.md must document config/learner asset"
        )

    def test_manifest_mentions_olav_netops(self):
        """Manifest must reference olav-netops as the target repo."""
        src = _NETOPS_MANIFEST.read_text(encoding="utf-8")
        assert "olav-netops" in src, (
            "NETOPS_WORKSPACE_MANIFEST.md must reference olav-netops as destination repo"
        )

    def test_manifest_not_empty(self):
        """Manifest must have substantive content (>200 chars)."""
        src = _NETOPS_MANIFEST.read_text(encoding="utf-8")
        assert len(src) > 200, "NETOPS_WORKSPACE_MANIFEST.md appears empty or trivial"


# ===========================================================================
# DOCS-1: docs/ ownership attribution
# ===========================================================================


class TestDocs1OwnershipAttribution:
    """docs/01_README.md must document repo ownership of docs subdirectories."""

    def test_docs_readme_exists(self):
        """docs/01_README.md must exist."""
        assert _DOCS_README.exists(), "docs/01_README.md not found"

    def test_docs_readme_mentions_olav_core(self):
        """docs/01_README.md must attribute docs/olav/ to olav-core."""
        src = _DOCS_README.read_text(encoding="utf-8")
        assert "olav-core" in src, "docs/01_README.md must indicate docs/olav/ belongs to olav-core"

    def test_docs_readme_mentions_olav_netops(self):
        """docs/01_README.md must attribute docs/netops/ to olav-netops."""
        src = _DOCS_README.read_text(encoding="utf-8")
        assert "olav-netops" in src, (
            "docs/01_README.md must indicate docs/netops/ belongs to olav-netops"
        )
...
class TestEnt1PackageBoundary:
    ...
    def test_enterprise_src_dir_exists(self):
        assert (_ROOT / "src" / "olav" / "enterprise").is_dir()

    def test_enterprise_init_exists(self):
        assert (_ROOT / "src" / "olav" / "enterprise" / "__init__.py").exists()
...
class TestTD32TrackingDocIndexPaths:
    """All file paths listed in tracking §6 design doc index must exist on disk."""
    ...
    def test_tracking_section6_stale_path_not_referenced(self):
        """tracking §6 must NOT reference the stale docs/cn/09_AGENTIC_FEATURES.md path."""
        tracking = _ROOT / "dev_docs" / "01. tracking.md"
        src = tracking.read_text(encoding="utf-8")
        assert "docs/cn/09_AGENTIC_FEATURES.md" not in src, (
            "TD-32: tracking §6 still references stale path docs/cn/09_AGENTIC_FEATURES.md. "
            "Update to docs/olav/cn/05_AGENTIC_FEATURES.md"
        )

    def test_tracking_section6_correct_cn_path_referenced(self):
        """tracking §6 must reference the correct docs/olav/cn/05_AGENTIC_FEATURES.md path."""
        pytest.xfail(
            "S16: v0.13 tracking §6 is 'Mandatory Evidence Template', "
            "no longer contains design doc index paths"
        )
...
_ENT_CLI_BRIDGE = _SRC / "olav" / "enterprise" / "cli_bridge.py"
