...
class TestToolGeneratorReadonlyFilter:
    ...
    def test_readonly_true_generates_only_get(self, tmp_path):
        """When readonly_only=True, only GET operations are written to tool file."""
        from olav.platform.services.tool_generator import _generate_group_tool_file

        svc = self._make_svc(readonly_only=True, output_dir=str(tmp_path))
        group = svc.tool_generation.groups[0]

        with patch("olav.platform.services.tool_generator._query_tag_operations",
                   return_value=self._fake_rows()):
            _generate_group_tool_file(svc, group)

        out = (tmp_path / "testsvc_labs.py").read_text()
        assert "get_labs" in out or "GET" in out
        assert "clab_post" not in out.lower()
        assert "clab_delete" not in out.lower()

    def test_readonly_false_generates_all_methods(self, tmp_path):
        """When readonly_only=False, all methods (GET/POST/DELETE) are generated."""
        from olav.platform.services.tool_generator import _generate_group_tool_file
        ...
        out = (tmp_path / "testsvc_labs.py").read_text()
        assert "post" in out.lower() or "create" in out.lower()
