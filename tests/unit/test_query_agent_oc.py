...
class TestSchemaReferenceOpenConfig:
    ...
    @pytest.mark.xfail(
        reason="OC-8 doc contract superseded: R82 rewrote SCHEMA_REFERENCE.md "
        "around the four canonical views and R83 auto-views replaced OpenConfig "
        "JSON-path querying; no workspace doc mentions openconfig anymore. "
        "Owner decision pending: retire OC-8 or re-spec against the R83 model.",
        strict=True,
    )
    def test_reference_contains_openconfig_section(self):
        from pathlib import Path

        ref_path = Path(".olav/workspace/core/references/SCHEMA_REFERENCE.md")
...
class TestSchemaReferenceSnapshotGuidance:
    ...
    def test_reference_does_not_use_parsed_outputs_snapshot_for_views(self):
        ...
        content = Path(".olav/workspace/core/references/SCHEMA_REFERENCE.md").read_text()
