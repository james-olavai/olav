...
class TestEmbeddingService:
    ...
    def service_with_key(self) -> EmbeddingService:
        ...
        with patch("olav.etl.document_indexer.settings") as mock_settings:
...
class TestDocumentIndexer:
    ...
    @pytest.fixture
    def indexer_and_mock(self, mock_client: MagicMock) -> Generator[tuple[DocumentIndexer, MagicMock], None, None]:
        """Create document indexer with mocked OpenSearch client."""
        with patch("olav.etl.document_indexer.create_opensearch_client", return_value=mock_client):
            with patch("olav.etl.document_indexer.settings") as mock_settings:
                mock_settings.opensearch_url = "http://localhost:9200"
                indexer = DocumentIndexer()
                yield indexer, mock_client

    @pytest.fixture
    def indexer(self, indexer_and_mock: tuple[DocumentIndexer, MagicMock]) -> DocumentIndexer:
        """Get just the indexer."""
        return indexer_and_mock[0]

    @pytest.fixture
    def mock_opensearch(self, indexer_and_mock: tuple[DocumentIndexer, MagicMock]) -> MagicMock:
        """Get just the mock client."""
        return indexer_and_mock[1]
    ...
    def test_custom_index_name(self, mock_client: MagicMock) -> None:
...
class TestRAGIndexer:
    ...
    def mock_services(self) -> Generator[dict, None, None]:
        ...
        with (
            ...
            patch("olav.etl.document_indexer.settings") as mock_settings,
        ):
            mock_settings.opensearch_url = "http://localhost:9200"
            mock_settings.llm_api_key = ""  # No API key = zero vectors
