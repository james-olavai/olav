...
class TestInitDocs:
    ...
    def test_creates_index_when_not_exists(self, mock_opensearch):
        ...
        with patch("olav.etl.init_docs.OpenSearch", return_value=mock_opensearch):
            with patch("olav.etl.init_docs.settings") as mock_settings:
                mock_settings.opensearch_url = "http://localhost:9200"

                from olav.etl.init_docs import main
                main()
