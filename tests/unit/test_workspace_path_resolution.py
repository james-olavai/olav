...
class TestGetActiveWorkspace:
    ...
    def test_reads_from_api_json(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        api = tmp_path / ".olav" / "config" / "api.json"
        api.parent.mkdir(parents=True)
        api.write_text(json.dumps({"active_workspace": "netops"}))
    ...
    def test_reads_from_settings_json(self, tmp_path, monkeypatch):
        """Legacy fallback: settings.json still works when api.json has no active_workspace."""
        monkeypatch.chdir(tmp_path)
    ...
    def test_api_json_takes_priority_over_settings_json(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        cfg = tmp_path / ".olav" / "config"
        cfg.mkdir(parents=True)
        (cfg / "api.json").write_text(json.dumps({"active_workspace": "ops"}))
        (cfg / "settings.json").write_text(json.dumps({"active_workspace": "legacy"}))
        from importlib import reload
        import olav.core.workspace as ws_mod
        reload(ws_mod)
        from olav.core.workspace import get_active_workspace
        assert get_active_workspace() == "ops"

    def test_tolerates_broken_api_json(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        api = tmp_path / ".olav" / "config" / "api.json"
        api.parent.mkdir(parents=True)
        api.write_text("not valid json {{")
        from olav.core.workspace import get_active_workspace
        assert get_active_workspace() == "core"
    ...
    def test_missing_key_returns_core(self, tmp_path, monkeypatch):
        ...
        api = tmp_path / ".olav" / "config" / "api.json"
        api.parent.mkdir(parents=True)
        api.write_text(json.dumps({"some_other_key": "value"}))
        from olav.core.workspace import get_active_workspace
        assert get_active_workspace() == "core"
