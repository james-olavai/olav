...
def test_add_user_creates_record(tmp_path) -> None:
    ...
    import asyncio
    result = asyncio.run(cmd.execute("add-user alice --role user --no-verify"))
...
def test_add_user_defaults_role_to_user(tmp_path) -> None:
    ...
    cmd = AdminUsersCommand(users_db=db)
    import asyncio
    asyncio.run(cmd.execute("add-user bob --no-verify"))
...
def test_add_user_admin_role(tmp_path) -> None:
    ...
    cmd = AdminUsersCommand(users_db=db)
    import asyncio
    asyncio.run(cmd.execute("add-user carol --role admin --no-verify"))
