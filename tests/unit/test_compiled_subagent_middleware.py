...
def _make_minimal_agent_build(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(agent_module, "build_summarization_middleware", lambda llm, **kw: fake_summ_instance)
...
class TestCompiledSubAgentMiddleware:
    ...
    def test_prompt_caching_middleware_when_available(self, tmp_path, monkeypatch):
        """CC-01: AnthropicPromptCachingMiddleware injected when provider is Anthropic."""
        ...
        # Simulate anthropic provider: patch should_use_prompt_caching to return True
        monkeypatch.setattr(agent_module, "should_use_prompt_caching", lambda: True)
    ...
    def test_prompt_caching_middleware_absent_for_non_anthropic(self, tmp_path, monkeypatch):
        """CC-01: AnthropicPromptCachingMiddleware NOT injected when provider is not Anthropic."""
        from olav.agents import _deepagents_bridge as bridge
        ...
        # Default env uses provider=custom — should_use_prompt_caching() returns False
        monkeypatch.setattr(agent_module, "should_use_prompt_caching", lambda: False)

        captured = _make_minimal_agent_build(tmp_path, monkeypatch)

        assert captured, "create_agent was not called"
        middleware_list = captured[0]
        assert not _has_middleware_type(middleware_list, AnthropicPromptCachingMiddleware), (
            "AnthropicPromptCachingMiddleware must NOT be injected for non-Anthropic providers"
        )
