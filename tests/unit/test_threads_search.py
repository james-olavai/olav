...
验收标准:
  ...
  4. AsyncSqliteSaver.list_threads() 可用且返回正确结构
"""
...
# ---------------------------------------------------------------------------
# Unit tests: AsyncSqliteSaver.list_threads()
# ---------------------------------------------------------------------------
...
class TestAsyncDuckDBSaverListThreads:
    ...
    def test_list_threads_returns_thread_id_and_updated_at(self, tmp_path):
        ...
        conn.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?, ?)",
            ["thread-abc", "", "2024-01-01T00:00:00.000000+00:00", None, "json", b'{}', b'{}'],
        )
        conn.commit()
    ...
    def test_list_threads_multiple_distinct_threads(self, tmp_path):
        ...
        for tid in ["thread-1", "thread-2", "thread-3"]:
            conn.execute(
                "INSERT INTO checkpoints (thread_id, checkpoint_ns, checkpoint_id, parent_checkpoint_id, type, checkpoint, metadata) "
                "VALUES (?, ?, ?, ?, ?, ?, ?)",
                [tid, "", "2024-01-01T00:00:00.000000+00:00", None, "json", b'{}', b'{}'],
            )
        conn.commit()
...
class TestThreadsSearchEndpoint:
    ...
    def test_search_threads_returns_non_empty_when_threads_exist(self, tmp_path, monkeypatch):
        ...
        conn.execute(
            "INSERT INTO checkpoints (thread_id, checkpoint_ns, checkpoint_id, parent_checkpoint_id, type, checkpoint, metadata) "
            "VALUES (?, ?, ?, ?, ?, ?, ?)",
            ["thread-xyz", "", "2024-01-01T00:00:00.000000+00:00", None, "json", b'{}', b'{}'],
        )
        conn.commit()
    ...
    def test_search_threads_returns_empty_list_when_no_threads(self, monkeypatch):
        ...
        with tempfile.TemporaryDirectory() as d:
            conn = sqlite3.connect(f"{d}/test.db", check_same_thread=False)
            saver = AsyncSqliteSaver(conn)
            saver.setup()
