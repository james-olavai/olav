...
class TestNetops1PhysicalMigration:
    ...
    @pytest.mark.skip(reason="NETOPS-1 not yet planned — ops workspace intentionally lives in monorepo")
    def test_monorepo_ops_workspace_removed(self):
        """After migration, .olav/workspace/ops/ must NOT exist in monorepo root."""
        monorepo_ops = _ROOT / ".olav" / "workspace" / "ops"
        assert not monorepo_ops.exists(), (
            f"ops workspace still found at monorepo root {monorepo_ops}. "
            "NETOPS-1: ops must be removed from monorepo after migration to olav-netops/"
        )
