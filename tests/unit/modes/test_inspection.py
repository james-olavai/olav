...
class TestInspectionResult:
    ...
    def test_to_markdown(self):
        ...
        # New format: "# 🔴 Daily Core Check — CRITICAL"
        assert "Daily Core Check" in md
        assert "CRITICAL" in md  # Status in header
        assert "1.0m" in md  # 60s = 1.0m
...
class TestDeviceResolution:
    ...
    async def test_resolve_empty_filter(self, controller):
        """Empty filter falls back to SuzieQ, returning devices from there."""
        df = DeviceFilter()

        # Mock the internal _resolve_from_suzieq to prevent real OpenSearch connection
        with patch.object(controller, "_resolve_from_suzieq", new_callable=AsyncMock) as mock_suzieq:
            mock_suzieq.return_value = []
            devices = await controller.resolve_devices(df)
            assert devices == []
            mock_suzieq.assert_called_once()
...
class TestCheckExecution:
    ...
    async def test_execute_check_unknown_tool(self, controller):
        ...
        assert result.success is False
        # Error occurs because the tool doesn't have required parameters
