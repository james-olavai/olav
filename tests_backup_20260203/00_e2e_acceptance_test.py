...
class TestCodeQuality:
    ...
    def test_pyright(self) -> None:
        ...
        # pyright 返回 0 表示无错误，1 表示有错误
        # 我们统计错误数量，但允许一些低优先级的类型警告

        output = result.stdout + result.stderr

        # Check for actual errors (lines containing " - error:")
        # Avoid false positives from warnings with "error" in parameter names
        error_lines = [l for l in output.split("\n") if " - error:" in l.lower()]

        if error_lines:
            print(f"\n❌ Pyright 发现类型错误 ({len(error_lines)} 个):")
            print("\n".join(error_lines[:15]))
            pytest.fail("Pyright 类型检查失败")
        elif result.returncode == 0:
            print("✅ Pyright 类型检查通过")
        else:
            print("⚠️  Pyright 发现类型警告（可能是低优先级的 Any 类型）")
            # 不失败，只警告（允许一些 Any 类型在动态语言中）
...
class TestPhase3ExportsStructure:
    ...
    def test_parsed_directories_exist(self, latest_snapshot: Path) -> None:
        """3.3 验证每个设备的 parsed 目录存在。"""
        for device in TEST_DEVICES:
            parsed_dir = latest_snapshot / "parsed" / device
            assert parsed_dir.exists(), f"缺少 parsed 目录: {device}"
...
class TestPhase7Inspection:
    ...
    def test_inspect_no_false_positives(self) -> None:
        """7.7 验证 inspect 不产生误报 (Critical: 0)。
        
        验证 BGP 'Active' 和 接口 'up' 不会被识别为 Critical 问题。
        此次测试基于已知的良性快照数据，因此不应有 Critical 报警。
        """
        result = subprocess.run(
            ["uv", "run", "olav", "inspect", "--group", "test"],
            capture_output=True,
        ...
        # 验证汇总部分，确保没有 Critical 问题
        # 输出可能包含 "- 🔴 Critical: 0"
        if "- 🔴 Critical: 0" not in result.stdout:
            print(f"⚠️  发现意外的 Critical 问题:\n{result.stdout[:1000]}")
            assert False, "Found unexpected Critical issues (False Positives)"
            
        print("✅ 验证通过: 无误报 (Critical: 0)")
...
class TestPhase7DeepAgents:
    ...
    涵盖:
    - Phase 7: Coder Agent (Template Developer)
    - Phase 8: Reflector (Self-Learning)
    - Phase 9: Planner (Intelligent Planning)
    """

    def test_coder_agent_textfsm_generation(self) -> None:
        """7.1 验证 Coder Agent 生成 TextFSM 模板的能力。
        
        模拟输入: 原始 CLI 输出
        期望: 生成并通过测试的 TextFSM 模板
        """
        ...
        intent = "extract interface and ip_address"
        
        # 这是一个长时间运行的任务
        # agent = create_coder_agent()
        # result = agent.invoke({"raw_output": raw_output, "intent": intent})
        
        # assert result['test_result'] is True
        # assert "Value INTERFACE" in result['current_template']
    ...
    def test_reflector_sop_extraction(self) -> None:
        ...
        """
        # history = [
        #     {"role": "user", "content": "R1 BGP down"},
        #     {"role": "agent", "content": "Checking neighbors..."},
        #     {"role": "tool", "content": "BGP state = Idle"},
        #     {"role": "agent", "content": "Root cause: ACL blocking 179"},
        # ]
        
        # sop = extract_sop(history)
        # assert "Check BGP Neighbors" in sop
        print("✅ Reflector Agent 测试桩通过 (待实现)")

    def test_planner_decomposition(self) -> None:
        """7.3 验证 Planner 将模糊意图分解为子任务。"""
        # from olav.agents.planner import plan_task
        
        # user_intent = "Check why R1 is unreachable"
        # plan = plan_task(user_intent)
        
        # assert len(plan) >= 2
        # assert plan[0].action == "check_connectivity"
