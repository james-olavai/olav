...
- `ent_dataset_redaction`
- `ent_dataset_sft`
- `ent_dataset_trajectory`
- `ent_dataset_atif`
- `ent_dataset_encryption`
- `ent_dataset_access_token`
- `ent_log_integrity`
- `ent_log_retention`
...
```yaml
enterprise_features:
  ent_dataset_redaction: true
  ent_dataset_sft: true
  ent_dataset_trajectory: true
  ent_dataset_atif: false
  ent_dataset_encryption: true
  ent_dataset_access_token: false
...
1. `ent_dataset_redaction`
  控制是否要求企业级字段脱敏策略参与导出 gate
2. `ent_dataset_encryption`
  控制是否要求导出产物满足加密策略
3. `ent_dataset_access_token`
  控制本地训练前是否需要一次性访问授权
