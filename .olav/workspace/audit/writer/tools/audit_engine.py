...
def _build_models():
    ...
    class AuditConfig(BaseModel):
        ...
        version: str = "1.0.0"
        description: str = ""
        type: Literal["health_check", "consistency", "compliance", "custom"] = "custom"
        created_at: str = ""
...
def load_audit_config(audit_name: str) -> Any:
    ...
    if not audit_yaml.exists():
        ...
        raise FileNotFoundError(f"Audit '{stem}' not found. Available audits: {available}")
    ...
    try:
        ...
        raise ValueError(f"Validation failed for '{stem}': {e}") from e
...
def _extract_computed(
    ...
    """
    si = _get_schema_inspector()
    num_field, den_field = si.discover_ratio_fields(
        target_def, rows, platform=platform, target_name=target_name
    )
...
def _extract_per_row(
    ...
    """
    si = _get_schema_inspector()
    key_field, value_field = si.discover_row_fields(
        target_def, rows, platform=platform, target_name=target_name
    )
        ...
        for record in parsed:
            ...
            if key and value:
                ...
                if key_str and key_str not in result:
                    ...
                    result[key_str] = si.normalize_value(
                        val_str, target_def, target_name=target_name
                    )
        ...
        if raw_text is None:
            ...
            return [
                AuditFinding(
                    ...
                    expected_value="raw snapshot file present",
                    severity="warning",
                    recommendation=(
...
def run_audit_reduce(
    ...
    has_raw_rules = any(r.condition in ("raw_contains", "raw_not_contains") for r in config.rules)
    if has_raw_rules:
    ...
    if warning_count > 0:
        recommendations.append(
            f"⚠️ WARNING: {warning_count} device(s) have warnings — review and monitor."
        )
    ...
    if per_device_cfg.enabled and llm:
        ...
        for device, dev_findings in by_device.items():
            ...
            try:
                ...
                logger.info("Per-device analysis generated for %s (%d chars)", device, len(text))
    ...
    if global_cfg.enabled and llm:
        ...
        for device, stat in device_stats.items():
            ...
            if per_text:
                ...
                device_summaries.append(
                    f"{icon} {device} ({stat.get('status', 'healthy').upper()}) — no findings."
                )
...
def generate_ai_analysis(result: dict[str, Any], config: Any) -> str:
    ...
    device_summary = "\n".join(
        f"  {d}: {s['status']} (score={s['health_score']}%, findings={s['anomaly_count']})"
...
def generate_audit_report(
    ...
    scope = ", ".join(devices_requested) if devices_requested else "All inventory devices"
    ...
    if _enabled("header"):
        ...
        lines += [
            ...
            "## Overall Health",
            "",
            f"**{_status_icon(overall)} {overall.upper()}** — {score:.0f}/100  `{overall_bar}`",
            "",
            "| Total | Healthy | Warning | Critical | Findings |",
            "|------|---------|---------|----------|----------|",
    ...
    if _enabled("device_health_table"):
        ...
        columns = (
            sec.columns
            if sec and sec.columns
            else ["device", "status", "health_score", "anomaly_count", "commands_inspected"]
        )
