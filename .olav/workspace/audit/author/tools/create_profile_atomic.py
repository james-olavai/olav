"""create_profile_atomic @tool — single-call Profile creation.
...
create_profile_atomic = StructuredTool.from_function(
    _create_profile_atomic,
    name="create_profile_atomic",
    description=(
        "Single-call Profile creation: introspects the DB, validates "
        "each SQL job against it, and writes the Profile YAML — all in "
        "one tool call. Use this for **creating** a new Profile. For "
        "extending an existing one, use `append_jobs` instead; for "
        "thresholds-only retuning, use `save_profile`. "
        "Returns {ok, profile_path, validation_errors?, introspection_used, message}."
    ),
    args_schema=_CreateProfileAtomicInput,
)
