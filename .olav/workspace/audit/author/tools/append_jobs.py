...
_append_jobs_tool = StructuredTool.from_function(
    ...
    name="append_jobs",
    description=(
        "Append new audit jobs to an existing Profile without overwriting "
        "existing jobs. Reads the current profile, validates schema + "
        "no-duplicate-name, and atomically re-saves with the new jobs "
        "appended. Each new job uses the canonical schema (name, type, "
        "severity, section_prompt, plus query for sql jobs or "
        "semantic_query/threshold for lancedb). Use after read_profile to "
        "see current coverage and after test_map_query to validate new SQL."
    ),
