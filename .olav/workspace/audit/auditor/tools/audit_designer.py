...
def validate_audit_config(audit_name: str) -> dict[str, Any]:
    ...
    for rule in config.rules:
        ...
        if rule.condition == "in_set" and not rule.allowed:
            errors.append(f"Rule '{rule.name}': condition 'in_set' requires 'allowed' list.")
        if rule.condition == "not_in_set" and not rule.forbidden:
            errors.append(f"Rule '{rule.name}': condition 'not_in_set' requires 'forbidden' list.")
...
def create_audit_config(
    ...
    for _f in sorted(_CONFIG_DIR.glob("*.yaml")):
        ...
        try:
            ...
            logger.debug("Failed to read '%s': %s", _f.name, exc)
            continue
    ...
    content = (
        example_content.replace("{{AUDIT_TITLE}}", title)
    ...
    try:
        with open(audit_yaml, "w", encoding="utf-8") as f:
            f.write("# Generated by OLAV Audit Agent\n")
