...
`runner` + `author` in rev 259. The `curator` sub-agent was removed in
rev N (2026-05-27) — all its scripts were dead code superseded by
view_builder (R83.2) and AuditMiddleware. Old `auditor` and `curator`
SKILL.md are no longer present — do not call `task("auditor", ...)` or
`task("curator", ...)`.
