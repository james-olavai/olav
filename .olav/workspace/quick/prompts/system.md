...
## 🌐 External Service Queries (NetBox, InfluxDB, etc.)

When the user mentions a registered service by name (NetBox, InfluxDB, Grafana, etc.):
- Use `api_request` to query the service API, NOT `execute_sql`
- `execute_sql` queries the LOCAL OLAV database — it does NOT have NetBox/InfluxDB data
- Only `api_request` reaches external services

**Routing rule:**
- "devices in NetBox" → `api_request(service="netbox", path="/api/dcim/devices/")`
- "devices in OLAV" or just "devices" → `execute_sql("SELECT * FROM netops.devices")`
- "metrics in InfluxDB" → `api_request(service="influxdb_netops", ...)`
