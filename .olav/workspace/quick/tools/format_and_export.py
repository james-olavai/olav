...
def format_and_export(
    ...
    Examples:
        ...
        {"path": "exports/agent_outputs/ospf_diagnosis.md", "size": 2048}
    """
    # 1. Determine output directory based on format
    from olav.core.config import AGENT_OUTPUTS_DIR, EXPORTS_DIR


    if format and format.lower() in ("csv", "json", "yaml", "yml"):
        output_dir = EXPORTS_DIR  # exports/
    elif format and format.lower() in ("md", "txt", "mmd"):
        output_dir = AGENT_OUTPUTS_DIR  # exports/agent_outputs/
    else:
    ...
    if output_dir is None:
        from olav.core.config import AGENT_OUTPUTS_DIR as _AGENT_OUTPUTS_DIR
        from olav.core.config import EXPORTS_DIR as _EXPORTS_DIR
        if format in ("csv", "json", "yaml", "yml"):
            output_dir = _EXPORTS_DIR  # exports/
        else:
            output_dir = _AGENT_OUTPUTS_DIR  # exports/agent_outputs/
    ...
    return {
        "path": str(filepath.relative_to(Path.cwd())),  # agent_outputs/xxx.csv
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("Testing format_and_export tool")
    print("=" * 80)

    # Test 1: Markdown report
    print("\n1️⃣ Testing Markdown report")
    md_content = """# 网络诊断报告
