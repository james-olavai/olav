...
async def run_e2e(
    ...
    # Resolve OC agent configs if scenario requests it
    oc_configs: dict[str, str] | None = None
    if plan.config_source == "oc_agent":
        _banner(3, "oc_agent: CABConfigExtractor → SRL configs")
        try:
            import sys as _sys
            _olav_src = Path(__file__).parents[4] / "src"
            if str(_olav_src) not in _sys.path:
                _sys.path.insert(0, str(_olav_src))
            from cab_config_extractor import CABConfigExtractor
            from clab_topology_render import build_iface_map_from_links
            import os as _os
            import duckdb as _duckdb
            _db_path = _os.environ.get(
                "OLAV_DB_PATH",
                str(Path(__file__).parents[4] / ".olav" / "databases" / "main.duckdb"),
            )
            _extractor = CABConfigExtractor(db_path=_db_path)
            _node_names = list(plan.nodes.keys())

            # Build iface_map from topology_links for involved nodes.
            # This translates real interface names (GigabitEthernet1) to SRL
            # ethernet-1/N names, consistent with the CLAB topology YAML.
            _iface_map: dict[str, str] = {}
            try:
                _placeholders = ", ".join(f"'{n}'" for n in _node_names)
                _tcon = _duckdb.connect(_db_path, read_only=True)
                _tlinks = _tcon.execute(f"""
                    SELECT source_interface, destination_interface
                    FROM topology_links
                    WHERE source_device IN ({_placeholders})
                       OR destination_device IN ({_placeholders})
                """).fetchall()
                _tcon.close()
                _iface_map = build_iface_map_from_links(
                    [{"source_interface": r[0], "destination_interface": r[1]} for r in _tlinks]
                )
            except Exception as _ie:
                print(f"  ⚠ iface_map build failed (non-fatal): {_ie}")

            oc_configs = _extractor.render_all_devices(
                change_intent=plan.change_intent or f"Deploy {plan.scenario_name}",
                blast_radius_devices=_node_names,
                iface_map=_iface_map,
            )
            print(f"  ✓ OC configs rendered for {list(oc_configs.keys())}")
            for _n, _cfg in oc_configs.items():
                _lines = [l for l in _cfg.splitlines() if l.strip() and not l.startswith("#")]
                print(f"    {_n}: {len(_lines)} set/ commands")
        except Exception as exc:
            print(f"  ⚠ oc_agent extraction failed (non-fatal, will skip config): {exc}")
            traceback.print_exc()
            oc_configs = {}

    # Step 3: Wait for exec API reachability (all containers alive)
    _banner(3, "wait_readiness (exec reachable)")
    try:
        ...
        if ready_result.status == "timeout":
            print("  ⚠ exec reachability timeout — nodes may not be ready for config push")
        else:
            n_ready = len([n for n in ready_result.nodes.values() if n.ssh_reachable])
            print(f"  ✓ {n_ready}/{len(deploy_result.nodes)} nodes exec-reachable")
    except Exception as exc:
        print(f"  ✗ wait_readiness exception: {exc}")
        traceback.print_exc()

    # Step 3b: Configure device (dry-run preview)
    ...
    # Step 5: Wait for protocol convergence (post-config health check)
    _banner(5, "wait_readiness (post-config convergence)")
    try:
        ready_result = await wait_readiness(
            deploy_result,
            evidence_dir=evidence_dir,
        ...
        if ready_result.status == "timeout":
            ...
            n_ready = len([n for n in ready_result.nodes.values() if n.ssh_reachable])
            print(f"  ✓ {n_ready}/{len(deploy_result.nodes)} nodes still reachable post-config")
    except Exception as exc:
        print(f"  ✗ wait_readiness exception: {exc}")
        traceback.print_exc()
