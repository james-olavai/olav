...
# Workspace directories that contain `tools/` subdirs. Anything discovered
# across these trees is introspectable via tool_help.
_TOOL_ROOTS = [
    _PROJECT_ROOT / ".olav" / "workspace" / "core" / "tools",
    _PROJECT_ROOT / ".olav" / "workspace" / "core" / "admin" / "tools",
    _PROJECT_ROOT / ".olav" / "workspace" / "core" / "api_query" / "tools",
    _PROJECT_ROOT / ".olav" / "workspace" / "core" / "db_query" / "tools",
    _PROJECT_ROOT / ".olav" / "workspace" / "core" / "remote" / "tools",
    _PROJECT_ROOT / ".olav" / "workspace" / "core" / "writer" / "tools",
    _PROJECT_ROOT / ".olav" / "workspace" / "ops" / "tools",
    _PROJECT_ROOT / ".olav" / "workspace" / "audit" / "auditor" / "tools",
    _PROJECT_ROOT / ".olav" / "workspace" / "audit" / "curator" / "tools",
    _PROJECT_ROOT / ".olav" / "workspace" / "services" / "tools",
]
...
def _discover_all_tools() -> list[Any]:
    ...
    for root in _TOOL_ROOTS:
        ...
        try:
            ...
            # Individual directory failures should not poison the whole lookup.
            continue
    return found
...
def _serialise_args(tool_obj: Any) -> list[dict[str, Any]]:
    ...
    for name, field in fields.items():
        ...
        # "required" detection varies across Pydantic versions.
...
# Maps SKILL.md ``tools_docstring_mode`` values to the canonical detail
# levels. ``compact`` is the v0.18 spec wording; ``brief`` is the arg
# name already exposed on tool_help — accept both to avoid a translation
# layer in every caller.
_SKILL_MODE_ALIASES = {
    "brief": "brief",
    "compact": "brief",
    "short": "brief",
...
def _read_agent_docstring_mode(agent_id: str) -> str | None:
    ...
    surprise them if the deployment tier shifts. Returns a canonical
    ``"brief"`` / ``"full"`` string, or ``None`` when the field is
    absent / unparseable / the SKILL.md itself is missing.
    """
    ...
    for root in _TOOL_ROOTS:
        if root.name != "tools":
            continue
        # Each tool root lives at `<agent_or_subagent>/tools/` so the
        # SKILL.md sits at `<agent_or_subagent>/SKILL.md`.
        agent_dir = root.parent
        if agent_dir.name == agent_id:
            candidates.append(agent_dir / "SKILL.md")

    # Also allow top-level agents (agent_dir may be one level up).
    ...
    for skill_md in candidates:
        ...
        if not text.startswith("---"):
            continue
        # Slice out the YAML frontmatter.
        parts = text.split("---", 2)
        if len(parts) < 3:
            continue
        try:
...
def _resolve_detail(detail: str | None, agent_id: str | None = None) -> str:
    ...
    Precedence (Round 49 adds step 2):

    1. Explicit ``detail="brief"|"full"`` wins.
    2. If ``agent_id`` is supplied, look up that agent's
       ``SKILL.md::tools_docstring_mode`` frontmatter (v0.18.1 spec).
       Accepts ``compact``/``brief``/``full``/``long``/``verbose``.
    3. Otherwise consult ``TIER_DEFAULTS[<tier>]["tool_help_detail"]``.
    4. Fallback: ``"full"`` so unconfigured environments keep legacy
       behaviour. Small/medium tiers get ``"brief"`` once the config
       lists ``tool_help_detail`` — no config churn needed at rollout.
    """
    ...
    try:
        ...
        # Default map: small/medium → brief, large → full.
        per_tier_default = {"small": "brief", "medium": "brief", "large": "full"}.get(
            tier, "full"
        )
