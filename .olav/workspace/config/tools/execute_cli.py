    ...
    try:
        ...
        if has_pipe and not pipe_allowed:
            return {
                "ok": False,
                "reason": (
                    f"Command '{base_cmd}' is blacklisted and does not allow pipe filtering. "
                    "Run execute_cli without '|' filter, or contact network operations to review blacklist."
                ),
            }
...
def get_nornir():
    ...
    # Lazy import — requires olav-netops: pip install olav-netops
    try:
        from nornir import InitNornir
    except ImportError as exc:
        raise ImportError(
            "nornir is not installed. Network CLI execution requires olav-netops.\n"
            "Install it with: pip install olav-netops"
        ) from exc
    ...
    def execute(self, device: str, command: str, timeout: int = 30) -> _ExecutionResult:
        try:
            ...
            try:
                ...
                raise ImportError(
                    "nornir-netmiko is not installed. Network CLI execution requires olav-netops.\n"
                    "Install it with: pip install olav-netops"
                ) from exc
