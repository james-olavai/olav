...
| Stage Final | `devops` agent + `run_python_code` | NetworkX 图分析 + Mermaid 生成 | edge_count 条图边 |
...
Step 3: 所有 HIGH-priority gaps 修复完成（或无 gaps）后:
  ...
  # Topology: 委托 devops agent 用 run_python_code + NetworkX 构建图
  # 示例: run_python_code("import networkx as nx; G = nx.Graph(); ...")
...
- **topology 最后**: 拓扑分析必须在所有 learner 修复完成后才调用（委托 devops agent + NetworkX）

### ❌ 禁止
- 调用 `collect_commands` 重新 SSH 来"刷新"已修复的命令（用 `reparse_outputs` 即可）
- 在 learner 修复未完成时调用 topology 分析
- 跳过拓扑分析（topology 是后续分析的基础）
