...
Also reloads:
  .olav/skills/olav-ops/config/allowed_commands.yaml   command whitelist
  .olav/skills/olav-ops/config/blacklisted_commands.yaml command blacklist
...
def _load_category_strategy() -> dict:
    ...
    import yaml
    config_path = PROJECT_ROOT / ".olav" / "workspace" / "config" / "sync" / "config" / "category_strategies.yaml"
...
def sync_commands() -> dict:
    ...
    config_dir = PROJECT_ROOT / ".olav" / "skills" / "olav-ops" / "config"
