...
Shared by: olav-admin, network-inspection
...
class InspectionScheduleInput(BaseModel):
    ...
    workflow: str = Field(
        ...
        description="Workflow name from .olav/workflows/"
    )
...
def _get_inspection_command(workflow: str) -> str:
    """Get the command to execute inspection"""
    return f"cd {PROJECT_ROOT} && {sys.executable} -m uv run olav inspect {workflow} >> {LOGS_DIR}/cron.log 2>&1"
...
def handle_schedule(workflow: str, schedule: str, enabled: bool) -> InspectionOutput:
    ...
    try:
        ...
        if job:
            ...
            # Create new job
            command = _get_inspection_command(workflow)
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return InspectionOutput(
                status="success",
                action="run",
                message=f"Inspection completed successfully",
                data={
                    "workflow": workflow,
...
def manage_inspection_schedule(
    ...
    """Manage network inspection schedules and execution.
    ...
    Args:
        ...
        workflow: Workflow name (default: "daily-inspection")
    ...
    Examples:
        # Schedule daily inspection at 6 AM
        manage_inspection_schedule("schedule", schedule="0 6 * * *")
        
        # Run inspection now
        manage_inspection_schedule("run")
        
        # Check status
        manage_inspection_schedule("status", days=7)
        
        # List all schedules
        manage_inspection_schedule("list")
    """
    try:
        # Validate input
