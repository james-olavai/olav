...
def _validate_command(command: str, platform: str | None = None) -> dict:
    ...
    try:
        ...
        if not rows:
            ...
            return {"ok": True, "warning": f"'{base_cmd}' not in commands registry; run sync_commands() to register it"}
    ...
    def execute(self, device: str, command: str, timeout: int = 30) -> _ExecutionResult:
        try:
            ...
            if not target.inventory.hosts:
                return _ExecutionResult(False, error=f"Device '{device}' not found in Nornir inventory")
...
class CLIExecutionOutput(BaseModel):
    ...
    warning: str | None = Field(
        None,
        description="Non-fatal warning (e.g., command not in registry)"
    )
