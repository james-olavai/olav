...
try:
    from config.paths import SYNC_DIR as _FW_SYNC_DIR  # noqa: F401
    _SYNC_DIR = _FW_SYNC_DIR
except ImportError:
    pass  # _SYNC_DIR already set above

try:
    from config.settings import get_settings as _fw_get_settings
...
def get_nornir() -> Nornir:
    ...
    if _nornir_instance is None:
        ...
        # Try settings first, then fall back to environment variables
        username = getattr(settings, "device_username", None) or os.getenv("DEVICE_USERNAME")
        password = getattr(settings, "device_password", None) or os.getenv("DEVICE_PASSWORD")
...
def sync_all(
    ...
    """
    from snapshot import take_snapshot as _take_snapshot

    # Invoke via .func to bypass LangChain tool wrapper for internal call
    return _take_snapshot.func(devices=devices, categories=categories, wait=False)
        ...
        for device_name in device_names:
            ...
            for txt_file in sorted(device_dir.glob("*.txt")):
                        ...
                        if not tmpl_content.strip():
                            ...
                            try:
                                ...
                                if fsm_rows:
                                    ...
                                    records = [
                                        dict(zip(headers, row)) for row in fsm_rows
                                    ]
                ...
                if parsed_data is None:
                    # Nothing parsed — raw is already on disk, skip DB insert.
                    skipped += 1
                    continue

                try:
                    # Check if row already exists before inserting
                    # (table may lack UNIQUE constraint)
                    existing = db.conn.execute(
                        "SELECT COUNT(*) FROM parsed_outputs "
                        "WHERE device_name=? AND command=? AND snapshot_date=?",
                        [device_name, command, sync_date_str],
                    ).fetchone()[0]
                    if existing == 0:
                        db.conn.execute(
                            ...
                            "VALUES (?, ?, ?, ?)",
                            [device_name, command, parsed_data, sync_date_str],
                        )
                        inserted += 1
                    else:
                        skipped += 1
