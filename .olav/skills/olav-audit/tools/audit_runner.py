...
def _query_parsed_outputs(devices: list[str] | None = None) -> list[dict[str, Any]]:
    ...
    try:
        where_clauses = ["snapshot_date = CURRENT_DATE"]
        params: list[Any] = []
...
def run_audit(
    ...
    logger.info(
        ...
        audit_name, config.type, len(config.rules), config.collect.intents,
    )
    ...
    if not sql_rows:
        ...
        _has_raw_files = _raw_dir.exists() and any(
            d for d in _raw_dir.iterdir() if d.is_dir()
        )
        if not (_has_raw_rules and _has_raw_files):
            return {
                "status": "no_data",
                "message": (
                    f"No parsed_outputs found for today (CURRENT_DATE). "
    ...
    summary = (
        f"{finding_count} findings across {device_count} devices. "
