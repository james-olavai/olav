...
def _llm_pick_field(
    ...
    candidate_lines = "\n".join(
        ...
        for name, score in candidates[:15]
    )
    prompt = (
    ...
    try:
        ...
        logger.debug(
            ...
            response, target_name, candidates[0][0],
        )
...
def _get_db_conn() -> Any:
    ...
    for candidate in [
        ...
        if candidate.exists():
            return duckdb.connect(str(candidate))
...
def discover_scalar_field(
    ...
    if best_score >= CONFIDENCE_THRESHOLD:
        logger.debug(
            ...
            best_field, best_score, CONFIDENCE_THRESHOLD, len(all_fields),
        )
...
def discover_ratio_fields(
    ...
    logger.debug(
        "ratio discover: numerator='%s', denominator='%s'", num_field, den_field
    )
...
def discover_row_fields(
    ...
    logger.debug(
        ...
        key_field, value_field, platform,
    )
        ...
        for device_name, device_rows in device_rows_map.items():
            ...
            try:
                if target_type == "scalar":
                    _prewarm_scalar(
                        target_name, target_def, platform, platform_overrides,
                        all_fields, description, llm, stats,
                    )
                elif target_type == "computed_ratio":
                    _prewarm_ratio(
                        target_name, target_def, platform, all_fields,
                        description, llm, stats,
                    )
                elif target_type == "per_row":
                    _prewarm_per_row(
                        target_name, target_def, platform, platform_overrides,
                        all_fields, matching_records, description, llm, stats,
                    )
                    _prewarm_values(
                        target_name, target_def, matching_records,
                        all_fields, description, llm, stats,
                        audit_config=audit_config,
                    )
            except Exception as exc:
                logger.debug("prewarm error for '%s' device '%s': %s", target_name, device_name, exc)
                stats["errors"] += 1

    logger.info(
        "prewarm_resolution_cache: cached=%d, llm_resolved=%d, skipped=%d, errors=%d",
        stats["cached"], stats["llm_resolved"], stats["llm_skipped"], stats["errors"],
    )
    return stats
