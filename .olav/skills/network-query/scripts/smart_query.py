...
def main(params: dict) -> dict:
    ...
    devices = (
        params.get("device") or params.get("devices") or params.get("host") or params.get("hosts")
    )
    ...
    if not device_list:
        return {"status": "error", "message": "Missing 'device' or 'devices' parameter"}
    ...
    for dev in device_list:
        try:
            ...
            if result.success:
                results.append(
                    {"device": dev, "data": result.output, "structured": result.structured}
                )
            else:
    ...
    if results:
        return {
            ...
            "data": results
            if len(results) > 1
            else results[0]["data"],  # Simplify for single device
    ...
    try:
        ...
        if not input_str:
            ...
            try:
                ...
                input_data = {}
                if device_match:
                    input_data["device"] = device_match.group(1)
                if command_match:
                    input_data["command"] = command_match.group(1).strip()
