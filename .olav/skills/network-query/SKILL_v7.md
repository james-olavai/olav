...
tools:
  - smart_sql_query
  - query_database  # Legacy fallback
  - inspect_schema  # Legacy fallback
...
prompts:
  system: |
    ...
    **Legacy Tools (Fallback):**
    - `query_database`: Direct SQL execution (no auto schema)
    - `inspect_schema`: Manual schema inspection
    
    Use these ONLY if smart_sql_query fails or for debugging.
...
### query_database(sql) [Legacy]

Direct SQL execution without auto schema.

**Use when:** debugging or smart_sql_query unavailable

### inspect_schema(table_name=None) [Legacy]

Manual schema inspection.

**Use when:** need detailed schema analysis beyond smart_sql_query context
