...
│   │   │   ├── llm.py          # ✅ [核心] LLM Factory (多厂商支持)
│   │   │   ├── memory.py       # ✅ OpenSearch 读写封装
│   │   │   ├── prompt_manager.py # ✅ 提示词模板管理器
│   │   │   ├── settings.py     # ✅ Pydantic Settings
│   │   │   └── inventory_manager.py # ✅ CSV 导入与配置同步
│   │   ├── agents/             # 智能体 (业务逻辑)
│   │   │   ├── __init__.py
...
│   │   │   ├── learner_agent.py# ✅ 自我学习节点 (SubAgent Factory)
│   │   │   └── middleware/     # ✅ 自定义中间件
│   │   │       ├── __init__.py
...
# 3. 创建 Config
mkdir -p config
touch config/{__init__.py,settings.py,nornir_config.yml,suzieq_config.yml}
...
# Core
touch $SRC/core/{__init__.py,graph.py,state.py,llm.py,memory.py}

# Agents
touch $SRC/agents/{__init__.py,root_agent.py,suzieq_agent.py,netconf_agent.py,rag_agent.py,learner_agent.py}

# Tools
touch $SRC/tools/{__init__.py,opensearch_tool.py,suzieq_tool.py,nornir_tool.py,datetime_tool.py}

# Execution
touch $SRC/execution/{__init__.py,sandbox.py,nornir_loader.py}

# ETL
touch $SRC/etl/{__init__.py,yang_parser.py,init_schema.py}
...
OLAV 使用 NetBox 作为唯一的设备清单来源，避免配置漂移。Nornir 和 SuzieQ 直接从 NetBox 动态同步设备。
