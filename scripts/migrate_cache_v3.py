...
to the existing semantic_cache table.
    ...
    try:
        ...
        for col_name, col_type in new_columns.items():
            if col_name not in existing_columns:
                try:
                    sql = f"ALTER TABLE commands.main.semantic_cache ADD COLUMN {col_name} {col_type}"
                    conn.execute(sql)
        ...
        if not added_columns:
            logger.info("No new columns needed (they might already exist)")
...
def verify_migration(db_path: str) -> bool:
    ...
    try:
        ...
        required_columns = ['agent', 'execution_plan', 'query_history', 'plan_version', 'success_rate']
        ...
        if sample and len(sample) > 4:
            logger.info(f"Sample record updated: agent={sample[4] if len(sample) > 4 else 'NULL'}, plan_version={sample[7] if len(sample) > 7 else 'N/A'}")
