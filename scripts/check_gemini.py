...
def check_gemini_in_path() -> bool | None:
...
def check_nvm_gemini() -> bool | None:
...
def check_shell_config() -> None:
    ...
    try:
        zsh_aliases = subprocess.run(
            ["grep", "-e", "^alias gemini", "~/.zshrc"], capture_output=True, text=True
        )
    ...
    try:
        bash_aliases = subprocess.run(
            ["grep", "-E", "^alias gemini", "~/.bashrc", "~/.bash_aliases"],
            capture_output=True,
            text=True,
        )
...
def test_gemini_execution() -> None:
    ...
    try:
        result = subprocess.run(
            ["~/.nvm/versions/node/v22.19.0/bin/gemini", "models", "list"],
            capture_output=True,
            text=True,
            timeout=10,
        )
...
def main() -> None:
    ...
    if not in_path and not nvm_available:
        ...
        print("2. Add to PATH:")
        print(
            '   - Add to ~/.bashrc: export PATH="\\$HOME/.nvm/versions/node/v22.19.0/bin:\\$PATH"'
        )
